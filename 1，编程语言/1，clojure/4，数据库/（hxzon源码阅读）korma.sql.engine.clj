;;（hxzon源码阅读）korma.sql.engine.clj
...
(defn sql-select [query]
  (let [clauses (map field-str (:fields query))    ;; 投影字段
        modifiers-clause (when (seq (:modifiers query))    ;; 修饰字符串
                           (str (reduce str (:modifiers query)) " "))
        clauses-str (utils/comma-separated clauses)    ;; 投影字符串
...
;; hxzon 核心
;; 返回一个query对象（映射），其中的 :sql-str 为生成的sql语句
(defn ->sql [query]
  (bind-params
   (case (:type query)
