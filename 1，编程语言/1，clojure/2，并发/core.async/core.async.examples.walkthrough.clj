;; core.async.examples.walkthrough.clj
;; core.async/examples/walkthrough.clj
...
;; 为了防止阻塞主线程，使用 thread ，它会在一个线程池里执行
...
;; 使用 go块 ，它会在一个特殊的线程池里执行
;; 阻塞型的通道操作，会暂停执行，而不是阻塞线程
;; 在 go块 里，使用另两个函数来读写
(let [c (chan)]
  (go (>! c "hello"))
  (assert (= "hello" (<!! (go (<! c)))))
...
(let [c1 (chan)
      c2 (chan)]
  (thread (while true
            (let [[v ch] (alts!! [c1 c2])] ;; hxzon：等待多个通道，只要其中一个可读
